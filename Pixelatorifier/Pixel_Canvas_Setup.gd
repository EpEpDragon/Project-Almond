...
func _ready():
	cam_perspective = get_tree().get_root().get_node("Spatial/Player")
...
func _process(delta):
	...
	self.get_material().set_shader_param("pos", Vector2(cam_perspective.translation.x, cam_perspective.translation.z))
